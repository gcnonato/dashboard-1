...
/**
 * Base class that represents a query for the 'anytv_category' table.
 *
 *
 *
 ...
 *
 * @method CategoryQuery leftJoinOffer($relationAlias = null) Adds a LEFT JOIN clause to the query using the Offer relation
 * @method CategoryQuery rightJoinOffer($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Offer relation
 * @method CategoryQuery innerJoinOffer($relationAlias = null) Adds a INNER JOIN clause to the query using the Offer relation
 *
 ...
 *
 * @method array findById(string $id) Return Category objects filtered by the id column
...
abstract class BaseCategoryQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'anytv_offers', $modelName = 'AnyTv\\Category', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `name`, `description` FROM `anytv_category` WHERE `id` = :p0';
        try {
    ...
    public function filterByOffer($offer, $comparison = null)
    {
        if ($offer instanceof Offer) {
            return $this
                ->addUsingAlias(CategoryPeer::ID, $offer->getCatId(), $comparison);
